...
extern void make_udp_reply_from_request(uint8_t*,char*,uint16_t,uint16_t);
...
extern void send_udp(uint8_t*,char*,uint16_t,uint16_t, uint8_t*, uint16_t);
extern void send_udp_broadcast(uint8_t*,uint16_t, uint16_t, uint16_t);
...
class TCPIP {
 public:
	...
	void makeUdpReply(uint8_t *buf,char *data,uint16_t len,uint16_t port){
...
#define clientSetHttpIp clientSetServerIp
	...
	void sendUdp(uint8_t *buf,char *data,uint16_t len,uint16_t sport, uint8_t *dip, uint16_t dport){
		send_udp(buf, data, len, sport, dip, dport);
	}

	void sendUdpBroadcast(uint8_t *buf, uint16_t len, uint16_t sport, uint16_t dport){
		send_udp_broadcast(buf, len, sport, dport);
	}
