...
/* CMO -- inclusion sale ici d'un morceau de header privé du bridge,
 * le but étant de pouvoir insérer les adresses macs des équipements derrières
 * les VC ATM même en mode shortcut.
 */
...
static void br2684_push(struct atm_vcc *atmvcc, struct sk_buff *skb)
{
	...
	/* CMO */
	/* ajout spécial : si on est dans le cas du bridge hardware vers une interface, 
	...
	if (brdev->to_dev) {
	  ...
	  /* dans le cas où le VC est inclus dans un bridge ethernet, 
	   * il est susceptible d'être bidirectionnel, et donc il faut renseigner le bridge sur 
	   * les adresses MAC qui sont derrière lui.
	   */
	  ...
	  static_p = p;
	  memcpy (static_mac_add_buf , skb->data +6, 6); 	  //	  br_fdb_insert(br, p , skb->data + 6, 0); // CMO -- début de l'adresse MAC DEST en position 6
