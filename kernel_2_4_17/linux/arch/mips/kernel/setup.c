...
#  if defined(CONFIG_BCM96338) // No adsl for MMS. TODO: find a better condition
#    define ADSL_SDRAM_IMAGE_SIZE 0
#  else
// #    include <../modulesrc/drivers/char/adsl/bcm96348/softdsl/AdslCoreDefs.h>
...
cpu_4kc:
	...
	case PRID_COMP_SIBYTE:
        ...
        case PRID_COMP_BRCM:
                ...
                switch (mips_cpu.processor_id & 0xff00) {
                case PRID_IMP_BCM6352:
                        mips_cpu.cputype = CPU_BCM6352;
                        break;
                case PRID_IMP_BCM6345:
                        mips_cpu.cputype = CPU_BCM6345;
                        break;
                case PRID_IMP_BCM6348:
                        mips_cpu.cputype = CPU_BCM6348;
                        break;
                default:
                        mips_cpu.cputype = CPU_UNKNOWN;
                        break;
...
static inline void parse_mem_cmdline(void)
{
	...
	for (;;) {
		...
		if (c == ' ' && !memcmp(from, "mem=", 4)) {
			...
			/* inv_xde : substract SDRAM memory size used by ADSL baseband */
			mem_size -= ADSL_SDRAM_IMAGE_SIZE;
