/*
 * $Id: redboot.c,v 1.2 2004/03/12 18:56:07 xde Exp $
 *
...
int parse_redboot_partitions(struct mtd_info *master, struct mtd_partition **pparts)
{
	...
	//printk("parse_redboot_partitions 0\n");
...
#endif
	//printk("parse_redboot_partitions 1\n");
	...
	for (i = 0; i < PAGE_SIZE / sizeof(struct fis_image_desc); i++) {
		...
		/* inv_xde */
		if (master->size == 0x00400000) {
		  printk("RedBoot partition for 32MBit aligned Flash\n");
		  buf[i].flash_base &= master->size-1;
		}
		else {
		  printk("RedBoot partition for 64MBit non aligned Flash\n");
		  buf[i].flash_base &= (master->size << 1) - 1;
		  buf[i].flash_base -= 0x00400000;
		}
		//		buf[i].flash_base &= master->size-1;
