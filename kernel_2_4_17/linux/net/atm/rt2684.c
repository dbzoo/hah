 ...
 */
static int rt2684_ioctl(struct atm_vcc *atmvcc, unsigned int cmd,
	unsigned long arg)
{
	int err;
...
extern int (*rt2684_ioctl_hook)(struct atm_vcc *, unsigned int, unsigned long);

/* the following avoids some spurious warnings from the compiler */
#define UNUSED __attribute__((unused))

static int __init UNUSED rt2684_init(void)
{
	struct proc_dir_entry *p;
	if ((p = create_proc_entry("rt2684", 0, atm_proc_root)) == NULL)
		return -ENOMEM;
	p->proc_fops = &rt2684_proc_operations;
	rt2684_ioctl_hook = rt2684_ioctl;
	return 0;
}

static void __exit UNUSED rt2684_exit(void)
{
	struct rt2684_dev *rtdev;
	rt2684_ioctl_hook = NULL;
