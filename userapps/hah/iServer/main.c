 ...
 */
void *sendBscQuery(Client *c, char *target)
{
  ...
  snprintf(buff, XAP_DATA_LEN, "xap-header\n"
           ...
           "{\n"
           "}\n",
           xapGetUID(), xapGetSource(), target);

  broadcastPacket(c, buff);
}

/** When the first xap-header of xap-hbeat message is seen
 * from the client we setup a directed TARGET filter
 *
...
int main(int argc, char *argv[])
{
  ...
  if(rv)
  {
    die_strerror("pthread_create() ret %d", rv);
  }
