 ...
 *
 * $Id: amd_flash.c,v 1.7 2004/09/23 08:02:49 ccth Exp $
 *
...
/* Support for Winbond W19B320SBT */
#if defined(CONFIG_BCM96338)
#define USE_DQ7_TO_TEST_ERASE_COMPLETE
#define USE_DQ7_TO_TEST_WRITE_COMPLETE
#endif
...
#define ADDR_DEVICE_ID2                 0x000E
#define ADDR_DEVICE_ID3                 0x000F
...
#define MANUFACTURER_WINBOND    0x22DA
...
#define AM29BDS643D2    0x2202
#define AM29BDS643D3    0x2200
#define AM29LV320DT     0x22F6
#define AM29LV320DB     0x22F9

#define AM29LV640MB    0x227E
#define AM29LV640MB2   0x2210
#define AM29LV640MB3   0x2200
...
/* Winbond */
#define W19B320SBT      0x222A


#define D6_MASK	0x40
#define D7_MASK	0x00000080
...
static inline int flash_is_busy(struct map_info *map, unsigned long addr,
				int interleave)
{
  // This is not used if USE_DQ7_TO_TEST_ERASE_COMPLETE is set
  __u32 read1, read2;
  ...
  read2 = wide_read(map, addr);
	...
	return ((read1 & D6_MASK) != (read2 & D6_MASK));
}
			  ...
			  const struct amd_flash_info *table, int table_size)
{
	...
	dev_id2 = wide_read(map, base + (map->buswidth * ADDR_DEVICE_ID2));
	dev_id3 = wide_read(map, base + (map->buswidth * ADDR_DEVICE_ID3));
	printk("mfr_id 0x%08X, dev_id 0x%08X, dev_id2 0x%08X, dev_id3 0x%08X\n", mfr_id, dev_id, dev_id2, dev_id3);
	...
	for (i = 0; i < table_size; i++) {
		if ((mfr_id == table[i].mfr_id) &&
		  ...
		  printk("Found %s\n", table[i].name);
		  printk("table[i] : mfr_id 0x%08X, dev_id 0x%08X, dev_id2 0x%08X, \
dev_id3 0x%08X, long_dev_id %d\n", table[i].mfr_id, 
			 table[i].dev_id, table[i].dev_id2, table[i].dev_id3, 
			 table[i].long_dev_id);
		  
		  if ((!table[i].long_dev_id) ||
		      ((table[i].long_dev_id == 1) && (dev_id2 == table[i].dev_id2) && (dev_id3 == table[i].dev_id3))) {
		    printk("Found %s\n", table[i].name);
...
static struct mtd_info *amd_flash_probe(struct map_info *map)
{
	...
	const struct amd_flash_info table[] = {
	{
		...
		long_dev_id:1,
		dev_id: AM29BDS643D,		
		dev_id2: AM29BDS643D2,
		dev_id3: AM29BDS643D3,
      ...
      numeraseregions: 2,
      regions: {
       { offset: 0x00000000, erasesize: 0x00010000, numblocks:  63 },
       { offset: 0x003F0000, erasesize: 0x00002000, numblocks:  8 }
      }
    }, {
      mfr_id: MANUFACTURER_AMD,
      dev_id: AM29LV320DB,
      ...
      numeraseregions: 2,
      regions: {
       { offset: 0x00000000, erasesize: 0x00002000, numblocks:  8 },
       { offset: 0x00010000, erasesize: 0x00010000, numblocks:  63 }
      }
    }, {
      mfr_id: MANUFACTURER_WINBOND,
      dev_id: W19B320SBT,
      name: "Winbond W19B320SBT",
      size: 0x00400000,
      numeraseregions: 2,
      regions: {
       { offset: 0x00000000, erasesize: 0x00002000, numblocks:  8 },
...
#if 1  
	{
      ...
      long_dev_id:1,
      dev_id: AM29LV640MB,
      dev_id2: AM29LV640MB2,
      dev_id3: AM29LV640MB3,
      name: "AMD AM29LV640MB",
      size: 0x00800000,
      numeraseregions: 3,
      regions: {
       { offset: 0x000000, erasesize: 0x10000, numblocks: 64 },
       { offset: 0x400000, erasesize: 0x02000, numblocks:  8 },
       { offset: 0x410000, erasesize: 0x10000, numblocks: 63 }
      }
    }
#else
	/* Fausse flash de 4M pour tests seulement. (CS0 seul) */
	{
      mfr_id: MANUFACTURER_AMD,
      long_dev_id:1,
      dev_id: AM29LV640MB,
	...
	printk("mtd_info :\ntype %d, size 0x%08X, erasesize 0x%08X, name %s, index %d, numeraseregions %d\n",
	       mtd->type, mtd->size, mtd->erasesize, mtd->name, mtd->index, mtd->numeraseregions);
	printk("mtd_info->priv : amd_flash_private : device_type %d, interleave %d, numchips %d, chipshift %d",
	       private->device_type, private->interleave, private->numchips, private->chipshift);
			  ...
			  unsigned long adr, __u32 datum)
{
	...
	int ret = 0;
	int times_left;
#ifdef USE_DQ7_TO_TEST_WRITE_COMPLETE
	__u32 read1;
#endif // #ifdef USE_DQ7_TO_TEST_WRITE_COMPLETE
...
retry:
	...
	times_left = 500000;
#ifdef USE_DQ7_TO_TEST_WRITE_COMPLETE
	read1 = wide_read(map, adr);
	while (times_left-- && ((datum ^ read1) & D7_MASK )) {
	  // While write is not complete, DQ7 is complementary of the written data
#else
...
#ifdef USE_DQ7_TO_TEST_WRITE_COMPLETE
		read1 = wide_read(map, adr);
		//		printk("Comparing read1 = 0x%08X and datum = 0x%08X\n", read1, datum);
#endif // #ifdef USE_DQ7_TO_TEST_WRITE_COMPLETE
	}
				  ...
				  unsigned long adr, u_long size)
{
	...
	struct amd_flash_private *private = map->fldrv_priv;
	DECLARE_WAITQUEUE(wait, current);
#ifdef USE_DQ7_TO_TEST_ERASE_COMPLETE
	__u32 read1;
#endif // #ifdef USE_DQ7_TO_TEST_ERASE_COMPLETE
...
retry:
...
#ifdef USE_DQ7_TO_TEST_ERASE_COMPLETE
	// Testing another "is busy?" method
	read1 = wide_read(map, adr);
	while (!(read1 & D7_MASK)) { // DQ7 goes up to 1 when erase is complete  // CCTH
#else
        while (flash_is_busy(map, adr, private->interleave)) {
#endif // #ifdef USE_DQ7_TO_TEST_ERASE_COMPLETE
		if (chip->state != FL_ERASING) {
		  printk("Someone's suspended the erase. Sleep\n");
	...
	/* Verify every single word */
	{
		...
		for (address = adr; address < (adr + size); address++) {
		  ...
		  //		  while ((verify = map->read8(map, address)) != 0xFF) ; // <- This works!
		...
		if (error) {
			...
			spin_unlock_bh(chip->mutex);
			printk(KERN_WARNING
			       "%s: verify error at 0x%x, size %ld, read 0x%08X.\n",
			       map->name, address, size, verify);
			DISABLE_VPP(map);
