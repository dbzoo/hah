...
static void *cramfs_read(struct super_block *sb, unsigned int offset, unsigned int len)
{
	...
	for (i = 0; i < BLKS_PER_BUF; i++) {
		...
		if (blocknr + i < devsize) {
			bh = getblk(sb->s_dev, blocknr + i,sb->s_blocksize);
...
#if 0
                        printk("Input data, size =%d\n",compr_len);
                        dumpHex((unsigned char *)p,compr_len);
#endif
...
#if 0
                        printk("Output data, size =%d\n",bytes_filled);
                        dumpHex((unsigned char *)pgdata,bytes_filled);
#endif
#endif		
			up(&read_mutex);
		}
