			         ...
			         struct sk_buff **pskb)
{
	...
	ret = find_nat_proto(proto)->exp_matches_pkt(exp, pskb);
	    ...
	    struct sk_buff **pskb)
{
	...
	if (helper) {
		...
		/* Helper might have to manip the packet even when there is no expectation */
		if (helper->mangle && helper->mangle(ct, ctinfo, pskb)) {
