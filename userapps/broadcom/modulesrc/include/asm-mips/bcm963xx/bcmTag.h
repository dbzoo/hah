...
//Michael: We don't want to show Broadcom string when user edit firmware image
...
#define RESERVED_LEN    (TAG_LEN-(2*TAG_VER_LEN)-BOARD_ID_LEN-(2*SIG_LEN)-SIG_LEN2- \
                        (4*IMAGE_LEN)-(3*ADDRESS_LEN)-(3*FLAG_LEN)-(3*TOKEN_LEN))
...
typedef struct _FILE_TAG
{
    ...
    unsigned char signiture_3[SIG_LEN2];         // additional info (RSA Signature)
    unsigned char reserved[RESERVED_LEN];        // reserved for later use
    unsigned char FlashLayoutVersion[TAG_VER_LEN];     //Michael:define 2, new add on 2.12 for prevent people downgrade from 2.12 to 2.10
    unsigned char fskernelValidationToken[TOKEN_LEN]; // vic add to check corrupted fskernel
