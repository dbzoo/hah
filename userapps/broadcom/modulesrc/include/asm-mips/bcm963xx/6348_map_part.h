...
typedef struct GpioControl {
  uint16        unused0;
  byte          unused1;
  byte          GPIODir_high; /* bits 36:32 */
  uint32        GPIODir;      /* bits 31:00 */
  uint16        unused2;
  byte          unused3;
  byte          GPIOio_high;  /* bits 36:32 */
...
#define GPIO_NUM_TO_MASK(X)             ( (X < 32) ? (1 << ((X) & GPIO_NUM_MAX_BITS_MASK)) : (0) )

/* Number to mask conversion macro used for GPIODir_high and GPIOio_high */
#define GPIO_NUM_MAX_BITS_MASK_HIGH     0x07
#define GPIO_NUM_TO_MASK_HIGH(X)        ( (X >= 32) ? (1 << ((X-32) & GPIO_NUM_MAX_BITS_MASK_HIGH)) : (0) )
