					 ...
					 struct net_device **dev_out)
{
	...
	*dev_out = NULL;
	...
	if (mmio_len < RTL_MIN_IO_SIZE) {
        ...
        printk("vendorid %x deviceid  %x pdev %x start %x , start 2 : %x bus %x irq %d \n",
	       pdev->vendor, pdev->device, pdev, pci_resource_start(pdev, 0), pci_resource_start(pdev, 1),1, pdev->irq);


	rc = pci_request_regions (pdev, "8139too");
	if (rc) {
	  DPRINTK("erreur pci_request_regions\n");
		goto err_out;
	}
