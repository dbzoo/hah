...
/* BFO: Annulation modifs BCM */
	if (sigsetsize != sizeof(sigset_t)) {
	       printk("*** sys_rt_sigaction: sigsetsize=%d sizeof(sigsett)=%dGoto out \n",sigsetsize,sizeof(sigset_t));
		goto out;
	}

	if (act) {
		if (copy_from_user(&new_sa.sa, act, sizeof(new_sa.sa))) {
		printk("sys_rt_sigaction: copy_from_user new_sa.sa=%d \n",sizeof(new_sa.sa));
			return -EFAULT;
		}
	}
	...
	//printk("ret=%d oact=0x%x \n",ret,oact);
	if (!ret && oact) {
		if (copy_to_user(oact, &old_sa.sa, sizeof(old_sa.sa))) {
			printk("copy_to_user sizeof(old_sa.sa=%d)\n",sizeof(old_sa.sa));
			return -EFAULT;
		}
	}
out:
