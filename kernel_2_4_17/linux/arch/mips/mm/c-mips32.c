...
mips32_dma_cache_inv_pc(unsigned long addr, unsigned long size)
{
	...
	if (size >= dcache_size) {
		...
		while (1) {
#if defined CONFIG_MIPS_BRCM
                        /* Should call invalidate, not flush */
...
{
        change_cp0_config(CONF_CM_CMASK, cm);
}
...
void __init ld_mmu_mips32(void)
{
	unsigned long config = read_32bit_cp0_register(CP0_CONFIG);
...
#if defined CONFIG_MIPS_BRCM
        {
                /* Enable caches */
                unsigned int res;
                res = read_32bit_cp0_register(CP0_DIAGNOSTIC);
                write_32bit_cp0_register(CP0_DIAGNOSTIC, (res | 0xC0000000));
        }
#endif
