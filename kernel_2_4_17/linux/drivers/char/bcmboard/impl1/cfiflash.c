...
static int flash_get_cfi(struct cfi_query *query, WORD *cfi_struct);
...
static WORD cfi_data_struct_29W160[] = {
...
/* application                                                       */ 
/*********************************************************************/
// move to from flash_init to globe for stack problem
                        ...
                        byte *buffer, int numbytes)
{
    ...
    for( i = 0; i < 3; i++ )
    {
        ret = flash_write(sector, offset, buffer, numbytes, FALSE);
                           ...
                           byte *buffer, int numbytes)
{
    ...
    for( i = 0; i < 3; i++ )
    {
        ret = flash_write(sector, offset, buffer, numbytes, TRUE);
        if( !memcmp( p, buffer, numbytes ) )
            break;
        ret = -1;
    }
...
int flash_get_cfi(struct cfi_query *query, WORD *cfi_struct)
{
    volatile WORD *fwp; /* flash window */
...
int flash_get_blk(int addr)
{
    ...
    if( i > relative_addr )
    {
        ...
        printk("Address does not start on flash block boundary. Use the current blk_start\n");
#endif       
