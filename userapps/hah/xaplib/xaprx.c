     ...
     for (i=0; i<strlen(a_buffer); i++) {
	  ...
	  case IN_KEYVALUE:
	       if (a_buffer[i]>=32) {
		    ...
		    strlcpy(g_xap_msg[g_xap_index].section, i_section, XAP_MAX_SECTION_LEN);
		    strlcpy(g_xap_msg[g_xap_index].name, i_keyname, XAP_MAX_KEYNAME_LEN);
		    strlcpy(g_xap_msg[g_xap_index].value, i_keyvalue, XAP_MAX_KEYVALUE_LEN);
		    if (g_debuglevel>=8) {
...
int xapmsg_getvalue(const char* a_keyname, char* a_keyvalue) {
     int i;
     const int composite_len = XAP_MAX_SECTION_LEN+XAP_MAX_KEYVALUE_LEN+1;
     char i_composite_name[composite_len];

     *a_keyvalue = '\0';
     for(i=0; i<g_xap_index; i++) {
	  strlcpy(i_composite_name, g_xap_msg[i].section, composite_len);
	  strlcat(i_composite_name,":", composite_len);
	  strlcat(i_composite_name, g_xap_msg[i].name, composite_len);
