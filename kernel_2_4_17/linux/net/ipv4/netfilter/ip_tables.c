...
/* Mysterious hacking, because the orignial macro does
 * not work with this kernel in MIPS gcc 3.0.1 compiled
 * with 2.4.3 kernel header, changing it to be inline
 * fixes it with gcc 3.0.1, but not gcc 3.0.4, so
 * here I change to be a real function. (it may affect performance a bit.)
 * 02/28/2002, Song Wang (songw@broadcom.com).
 */
static int FWINV(int bool, int flag1, int flag2)
{
...
/*#define FWINV(bool,invflg) ((bool) ^ !!(ipinfo->invflags & invflg)) */
	...
	if (FWINV((ip->saddr&ipinfo->smsk.s_addr) != ipinfo->src.s_addr,
		  ipinfo->invflags,IPT_INV_SRCIP)
	    || FWINV((ip->daddr&ipinfo->dmsk.s_addr) != ipinfo->dst.s_addr,
		     ipinfo->invflags,IPT_INV_DSTIP)) {
	...
	if (FWINV(ret != 0, ipinfo->invflags,IPT_INV_VIA_IN)) {
	...
	if (FWINV(ret != 0, ipinfo->invflags,IPT_INV_VIA_OUT)) {
	...
	if (ipinfo->proto
	    && FWINV(ip->protocol != ipinfo->proto,  ipinfo->invflags,IPT_INV_PROTO)) {
	 ...
	 * then we return zero */
	if (FWINV((ipinfo->flags&IPT_F_FRAG) && !isfrag, ipinfo->invflags, IPT_INV_FRAG)) {
...
static int __init init(void)
{
	...
	dprintf("ip_tables: (c)2000 Netfilter core team\n");
	return 0;
}
